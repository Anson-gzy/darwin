...
# and the skill entry points. Bulk asset directories stay out, since importing content
...
if [ "$unlogged" -gt 0 ] && [ "${SYNC_ALLOW_UNLOGGED:-0}" != "1" ]; then
  ...
  governed >&2
  echo "sync: run darwin.sh <action> <file> <one-line-why> on each, then sync again." >&2
